...
GROUP = {
    ...
    "product_group_l2_code": "03.01",
    "product_group_l2_name": "Juoma-astiat",
    "product_group_l3_code": "3.1.2001",
    "product_group_l3_name": "Juomapullot ja termospullot",
}
...
def main():
    ...
    backup_csv = CSV_PATH.replace(
        ".csv", f".backup_before_urheilupullo_group_{timestamp}.csv"
    )
    audit_csv = os.path.join(OUTPUT_DIR, "urheilupullo_group_update_rows.csv")
    audit_json = os.path.join(OUTPUT_DIR, "urheilupullo_group_update_audit.json")

    shutil.copy2(CSV_PATH, backup_csv)
    df = pd.read_csv(CSV_PATH, low_memory=False)

    mask = df["name"].astype(str).str.contains("Urheilupullo", case=False, na=False)
    before = df.loc[mask].copy()

    for col, value in GROUP.items():
        df.loc[mask, col] = value
    df.loc[mask, "product_group_match_method"] = "manual_urheilupullo_rule"
