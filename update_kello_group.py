...
def main():
    ...
    backup_csv = CSV_PATH.replace(".csv", f".backup_before_kello_group_{timestamp}.csv")
    audit_csv = os.path.join(OUTPUT_DIR, "kello_group_update_rows.csv")
    audit_json = os.path.join(OUTPUT_DIR, "kello_group_update_audit.json")

    shutil.copy2(CSV_PATH, backup_csv)
    df = pd.read_csv(CSV_PATH, low_memory=False)
    before_missing = ~has_group(df)

    mask = df["name"].astype(str).str.contains("kello", case=False, na=False)
    before = df.loc[mask].copy()

    for col, value in GROUP.items():
        df.loc[mask, col] = value
    df.loc[mask, "product_group_match_method"] = "manual_kello_keyword_rule"
    ...
    after_missing = ~has_group(df)
    before["sales_num"] = pd.to_numeric(before["sales"], errors="coerce").fillna(0)
    summary = {
        "updated_at": datetime.now().isoformat(timespec="seconds"),
        "source_csv": CSV_PATH,
        "backup_csv": backup_csv,
        "matched_rows": int(mask.sum()),
        "matched_rows_missing_group_before": int((mask & before_missing).sum()),
        "matched_sales_eur": float(before["sales_num"].sum()),
        "top_names": {k: int(v) for k, v in before.groupby("name").size().sort_values(ascending=False).head(25).to_dict().items()},
        "previous_group_counts": {k: int(v) for k, v in before["product_group_l3_name"].fillna("").value_counts().to_dict().items()},
        "assigned_group": GROUP,
        "rows_missing_l3_after_all": int(after_missing.sum()),
