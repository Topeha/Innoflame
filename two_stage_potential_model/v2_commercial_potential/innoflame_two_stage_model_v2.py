...
DEFAULT_OUTPUT = Path("outputs") / "innoflame_two_stage_v2" / "innoflame_two_stage_potential_v2.csv"
DEFAULT_MARKET_POTENTIAL = (
    Path("outputs") / "innoflame_all_accounts" / "prospect_segment_model_all_accounts.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Two-stage Innoflame v2 model: forecast + calibrated commercial potential."
    )
    ...
    parser.add_argument("--market-potential", default=str(DEFAULT_MARKET_POTENTIAL))
...
def load_market_potential(path: str | Path) -> pd.DataFrame:
    market = pd.read_csv(path)
    required = {"business_id", "ennustettu potentiaali"}
    missing = required.difference(market.columns)
    if missing:
        raise ValueError(f"Market potential file is missing columns: {sorted(missing)}")
    keep_columns = ["business_id", "ennustettu potentiaali"]
    optional_columns = [column for column in ["score", "positive_signals"] if column in market.columns]
    market = market[keep_columns + optional_columns].copy()
    market = market.rename(
        columns={
            "ennustettu potentiaali": "market_potential_eur",
            "score": "market_model_score",
        }
    )
    market["market_potential_eur"] = pd.to_numeric(market["market_potential_eur"], errors="coerce").fillna(0)
    aggregation: dict[str, str] = {"market_potential_eur": "max"}
    if "market_model_score" in market.columns:
        aggregation["market_model_score"] = "max"
    if "positive_signals" in market.columns:
        aggregation["positive_signals"] = "first"
    return market.groupby("business_id", as_index=False).agg(aggregation)


def commercial_priority(rank: pd.Series) -> pd.Series:
    return pd.cut(
        rank,
...
def calculate_commercial_potential(output: pd.DataFrame) -> pd.Series:
    expected = output["expected_12m_sales_eur"].fillna(0).clip(lower=0)
    buyer = output["predicted_buyer_potential_eur"].fillna(0).clip(lower=0)
    market = output["market_potential_eur"].fillna(0).clip(lower=0)
    segment_lift = output["segment_lift"].fillna(0)
    revenue_k = output["revenue_k_eur"].fillna(0)

    blended = (0.50 * expected) + (0.30 * buyer) + (0.20 * market)
    boost = np.select(
        [
            (segment_lift >= 5) & (revenue_k >= 20_000),
            (segment_lift >= 3) | (revenue_k >= 100_000),
            (segment_lift >= 1.5) | (revenue_k >= 20_000),
        ],
        [1.25, 1.15, 1.08],
        default=1.0,
    )
    commercial = blended * boost

    strong_fit = (segment_lift >= 2.0) | (revenue_k >= 20_000)
    probability = output["purchase_probability"].fillna(0)
    floors = np.select(
        [
            strong_fit & (probability >= 0.55),
            strong_fit & (probability >= 0.35),
            strong_fit & (probability >= 0.20),
        ],
        [5_000, 2_500, 1_000],
        default=0,
    )
    commercial = np.maximum(commercial, floors)

    # Keep the commercial potential below the old market-potential ceiling when that exists.
    ceiling = np.where(market > 0, market, np.maximum(commercial, buyer))
    return pd.Series(np.minimum(commercial, ceiling), index=output.index).round().clip(lower=0)
...
def main() -> None:
    ...
    output["expected_12m_sales_eur"] = output["purchase_probability"] * output["predicted_buyer_potential_eur"]
    output["ennustettu_12kk_myynti_eur"] = output["expected_12m_sales_eur"].round().clip(lower=0).astype(int)

    market_potential = load_market_potential(args.market_potential)
    output = output.merge(market_potential, on="business_id", how="left")
    output["market_potential_eur"] = output["market_potential_eur"].fillna(0)
    output["kaupallinen_potentiaali_eur"] = calculate_commercial_potential(output).astype(int)
    output["ennustettu potentiaali"] = output["kaupallinen_potentiaali_eur"]
    output["forecast_rank"] = output["expected_12m_sales_eur"].rank(method="first", ascending=False).astype(int)
    output["commercial_rank"] = output["kaupallinen_potentiaali_eur"].rank(method="first", ascending=False).astype(int)
    output = output.sort_values(["commercial_rank", "purchase_probability"], ascending=[True, False]).reset_index(drop=True)
    output["rank"] = output["commercial_rank"]
    output["priority"] = commercial_priority(output["commercial_rank"])
    output["forecast_priority"] = rank_priority(output["forecast_rank"])
    ...
    output_columns = [
        ...
        "expected_12m_sales_eur",
        "ennustettu_12kk_myynti_eur",
        "market_potential_eur",
        "kaupallinen_potentiaali_eur",
    ...
    metrics = {
        ...
        **potential_metrics,
        ...
        "total_expected_12m_sales_eur": int(output["ennustettu_12kk_myynti_eur"].sum()),
        "customer_expected_12m_sales_eur": int(customer_only["ennustettu_12kk_myynti_eur"].sum()),
        "total_commercial_potential_eur": int(output["kaupallinen_potentiaali_eur"].sum()),
        "customer_commercial_potential_eur": int(customer_only["kaupallinen_potentiaali_eur"].sum()),
        "total_market_potential_eur": int(output["market_potential_eur"].sum()),
        "customer_market_potential_eur": int(customer_only["market_potential_eur"].sum()),
        "commercial_potential_formula": "min(market_potential, 0.50*expected_12m + 0.30*buyer_potential + 0.20*market_potential, with segment/revenue boosts and floors)",
